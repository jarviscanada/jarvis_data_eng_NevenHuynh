...
# Start docker
# Checks the status of docker and if is it not running, then start docker
...
case $cmd in
  ...
  # If docker run succeeds, then exit with error code 0. Otherwise, exit with error code 1
	exit $?
	;;
